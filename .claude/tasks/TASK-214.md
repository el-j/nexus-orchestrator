...
3. In `useTasks.ts`: replace hardcoded `http://127.0.0.1:9999` with a configurable base URL from Wails runtime or environment. Consider a shared `useConfig()` composable that reads from Wails binding
