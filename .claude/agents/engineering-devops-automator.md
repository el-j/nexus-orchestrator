...
color: orange
emoji: ⚙️
vibe: Reusable GitHub Actions workflows so every el-j repo ships faster.
...
**Node/TypeScript/Vue** — use Biome (preferred): `biome.json` with `indentStyle: "space"`, `recommended: true`, `vcs.useIgnoreFile: true`. Alternative: ESLint + Prettier.

**Go** — `gofmt -l .` + `golangci-lint run` with `.golangci.yml` enabling: `gofmt`, `govet`, `errcheck`, `staticcheck`, `gosimple`, `ineffassign`, `typecheck`.
...
2. Create `.github/workflows/ci.yml` calling the matching shared workflow via `uses: el-j/.github/.github/workflows/ci-go.yml@main` (with appropriate `with:` inputs like `go-version: '1.24'`). Trigger on push to `main`, `feature/*`, `copilot/*` and on pull_request.
3. Create `.github/workflows/deploy.yml` if a Pages site exists
...
6. **If the repo ships a service/daemon**: add `build-push-docker.yml` to CI (no-push) and `release-docker.yml` to the release pipeline

### Docker / GHCR

**Image naming**: `ghcr.io/el-j/<repo-name>:<tag>` — always use the GitHub repository name, lowercase.

**CI build check (no push on PRs)**: call `build-push-docker.yml@main` with `image-name: <repo-name>` and `push: false`.

**Release push (on v\* tags)**: call `release-docker.yml@main` with `image-name: <repo-name>`.

**Required permissions**: `packages: write` (set in **Settings → Actions → General** or org-level).

**Kubernetes pull secret** (one-time per namespace): create a PAT with `read:packages`, then `kubectl create secret docker-registry ghcr-secret --docker-server=ghcr.io --docker-username=el-j --docker-password=<PAT> --namespace=<namespace>`. Reference via `imagePullSecrets: [{name: ghcr-secret}]` in the pod spec.

Use `.claude/commands/docker-setup.md` to scaffold Docker for a new repo.
Use `.claude/commands/k8s-pull-secret.md` to generate the k8s secret commands.
