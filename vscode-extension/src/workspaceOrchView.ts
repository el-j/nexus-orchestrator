...
class LiveActivityGroupNode extends vscode.TreeItem {
  readonly kind = 'liveActivity' as const;
  constructor(
    public readonly activities: AIActivity[],
    public readonly workspacePath: string,
  ) {
    super(`Live Activity (${activities.length})`, vscode.TreeItemCollapsibleState.Expanded);
    this.iconPath = new vscode.ThemeIcon('pulse');
    this.contextValue = 'nexusLiveActivity';
  }
}

class ActivityItemNode extends vscode.TreeItem {
  readonly kind = 'activityItem' as const;
  constructor(public readonly activity: AIActivity) {
    ...
    const emoji = emojiMap[activity.activityType] ?? '•';
    const ageStr = relativeTime(new Date(activity.timestamp));
    super(`${emoji} ${activity.summary}`, vscode.TreeItemCollapsibleState.None);
    this.description = `${activity.agentName} · ${ageStr}`;
    this.tooltip = `${activity.agentName}\n${activity.summary}\n${activity.projectPath ?? ''}`;
    this.iconPath = new vscode.ThemeIcon('circle-small');
    this.contextValue = 'nexusActivityItem';
  }
}
...
export class WorkspaceOrchViewProvider implements vscode.TreeDataProvider<OrchNode> {
  ...
  async getChildren(element?: OrchNode): Promise<OrchNode[]> {
    ...
    if (element.kind === 'folder') {
      ...
      if (this.client) {
        // Live activities — shown first
        try {
          const folderPath = element.orch.folderPath;
          const activities = await this.client.getActivities({
            projectPath: folderPath,
            limit: 10,
          });
          this.liveActivities.set(folderPath, activities);
          if (activities.length > 0) {
            children.push(new LiveActivityGroupNode(activities, folderPath));
          }
        } catch {
        ...
        try {
          ...
          const projectTasks = allTasks.filter(
            (t) => t.projectPath.replace(/\/$/, '') === folderNorm,
          );
    ...
    if (element.kind === 'liveActivity') {
      return element.activities.map((a) => new ActivityItemNode(a));
    }
