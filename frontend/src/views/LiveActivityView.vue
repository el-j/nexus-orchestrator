  ...
  <div class="flex flex-col h-full overflow-hidden">
    ...
    <div class="flex-1 overflow-auto p-5 space-y-6">
      ...
      >
        <div class="flex items-center gap-2 mb-3">
          ...
          <span class="text-xs font-semibold text-emerald-400 uppercase tracking-wider"
            >Actively Generating</span
          >
        </div>
        <div class="flex flex-wrap gap-2">
          <div
            v-for="p in generating"
            :key="p.id"
      ...
      <section>
        ...
        <div v-if="activeSessions.length === 0" class="text-xs text-slate-600 italic py-2">
          ...
          <span class="text-slate-500"
            >Connect VS Code Copilot or an MCP client to see them here.</span
          >
        </div>
      ...
      <section>
        ...
        <div
          v-if="loading && discovered.length === 0"
...
const generating = computed(() => discovered.value.filter((p) => p.generating));

const activeCount = computed(() => activeSessions.value.length + generating.value.length);
