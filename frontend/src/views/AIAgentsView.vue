  ...
  <div class="flex flex-col h-full overflow-hidden">
    ...
    >
      ...
      <div class="flex items-center gap-2">
        ...
        >
          <i
            :class="['pi pi-arrow-right-arrow-left text-xs mr-1', delegatingAll && 'animate-spin']"
          ></i>
    ...
    <div class="flex-1 overflow-auto p-5">
      ...
      <!-- Empty state -->
      <div
        v-else-if="sessions.length === 0 && discovered.length === 0"
      ...
      <template v-else>
        ...
        <section v-if="sessions.length > 0" class="mb-6">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <AISessionCard v-for="s in sessions" :key="s.id" :session="s" @delegate="delegate" />
          </div>
        </section>
  ...
  try {
    ...
    if (data.instruction) {
      ...
      toast.add({
        severity: 'success',
        summary: 'Delegated',
        detail: 'Instruction copied to clipboard',
        life: 3000,
      });
    } else {
