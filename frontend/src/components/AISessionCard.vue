<template>
  ...
  >
    ...
    <details @toggle="handleToggle">
      <summary class="text-[11px] text-slate-500 cursor-pointer hover:text-slate-300 select-none list-none outline-none">
        {{ open ? 'Hide tasks ▲' : 'Show tasks ▼' }}
      </summary>
      <div v-if="open" class="mt-2">
        <div v-if="loadingTasks" class="text-[11px] text-slate-500 py-2">Loading…</div>
        <div v-else-if="sessionTasks.length === 0" class="text-[11px] text-slate-600 py-2">No tasks yet</div>
        <div v-else class="space-y-0.5">
          ...
          >
            <span :class="`text-[9px] px-1.5 py-0.5 rounded font-medium flex-shrink-0 ${statusChipClass(task.status)}`">
              {{ task.status }}
            </span>
            <span class="text-[10px] text-slate-400 font-mono truncate flex-1 min-w-0">{{ task.targetFile }}</span>
            <span class="text-[10px] text-slate-500 truncate max-w-[120px] flex-shrink-0">
              {{ (task.instruction ?? '').slice(0, 80) }}…
            </span>
            <span class="text-[10px] text-slate-600 flex-shrink-0 whitespace-nowrap">{{ relativeTime(task.updatedAt) }}</span>
          </div>
        </div>
      </div>
    </details>
...
function statusChipClass(status: TaskStatus | undefined): string {
  switch (status) {
    ...
    case 'QUEUED': return 'bg-violet-500/20 text-violet-300'
    case 'FAILED': return 'bg-red-500/20 text-red-300'
    case 'CANCELLED': return 'bg-slate-700/50 text-slate-400'
    case 'DRAFT':
    case 'BACKLOG': return 'bg-yellow-500/15 text-yellow-300'
...
async function openTimeline() {
  ...
  if (typeof EventSource !== 'undefined') {
    ...
    } catch { /* EventSource not supported for this endpoint */ }
  }
}
